...
from model.Net.NTRENet_plus import Network   
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = Network(layers=args.layers, classes=2, zoom_factor=8, \
    ...
    train_sampler = None
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=args.batch_size, shuffle=(train_sampler is None), num_workers=args.workers, pin_memory=True, drop_last=True)
    if args.evaluate:
        if args.resized_val:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch ==0:
            ...
            # max_prototype_neg_dict = None
        # if epoch == 10:
            max_prototype_neg_dict = dict()
        epoch_log = epoch + 1
        loss_train, mIoU_train, mAcc_train, allAcc_train,prototype_neg_dict,max_prototype_neg_dict = train(train_loader, model, optimizer, epoch, train_data.sub_list,\
            prototype_neg_dict=prototype_neg_dict,max_prototype_neg_dict=max_prototype_neg_dict)
        if main_process():
...
def train(train_loader, model, optimizer, epoch, base_list,prototype_neg_dict=None,max_prototype_neg_dict=None):
    ...
    for i, (input, target, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss, prototype_neg_dict,max_prototype_neg_dict= model(s_x=s_input, s_y=s_mask, x=input, y=target, classes = subcls, prototype_neg_dict=prototype_neg_dict,max_prototype_neg_dict=max_prototype_neg_dict)
    ...
    return main_loss_meter.avg, mIoU, mAcc, allAcc, prototype_neg_dict,max_prototype_neg_dict
