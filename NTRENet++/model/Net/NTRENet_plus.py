...
from model.prototypical_contrast_withQGT import PrototypeContrastLoss
from model.maxprototypical_contrast_pixel2pixel_paddn import MAXPrototypeContrastLoss
from model.ASPP import PSPnet
...
class Network(nn.Module):
    ...
    def forward(self, s_x, s_y,x, y, classes, prototype_neg_dict=None, max_prototype_neg_dict=None):
        ...
        corr_query_mask = self.generate_prior(query_feat_4, final_supp_list, mask_list, supp_feat_3.size(-1))
        ...
        if self.training:
            ...
            maxprototype_contrast_loss, max_prototype_neg_dict = self.maxcontrast_loss(query_feat, supp_feat_nomask, out.detach(), \
                y, s_y, query_bg_out, supp_bg_out, classes, max_prototype_neg_dict)
        ...
        if self.training:
            ...
            aux_bg_loss = torch.zeros_like(main_loss).cuda()    
            ...
            return out.max(1)[1], main_loss,bg_loss+0.4*aux_loss1+0.6*aux_loss2+0.01*prototype_contrast_loss,prototype_neg_dict,max_prototype_neg_dict
        else:
            return out
