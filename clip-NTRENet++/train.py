...
def get_arguments():
    ...
    parser.add_argument("--arch", type=str, default='clip_NTRENet_plus')
...
def train(args):
    ...
    net = eval(args.arch).Network()
    print('    Total params: %.2fM' % (sum(p.numel() for p in net.parameters()) / 1000000.0))

    for param in net.encoder.parameters():
        param.requires_grad = False

    optimizer = net.get_optimizer(args)
    net = net.cuda()
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        for iter, data in enumerate(train_loader):
            ...
            classes = idx.unsqueeze(-1).expand(-1,args.query_frame)
            pred_map,query_bg_out,supp_bg_out,aux_loss,prototype_neg_dict,max_prototype_neg_dict = net(query_img, query_mask, support_img, support_mask, classes.detach(), prototype_neg_dict=prototype_neg_dict, max_prototype_neg_dict=max_prototype_neg_dict)
            ...
            bg_loss = net.bg_loss(query_bg_out,supp_bg_out,query_mask,support_mask)
      
            total_loss = 5 * few_ce_loss + few_iou_loss + bg_loss + aux_loss
            # print('ce_loss:',few_ce_loss.item(),'iou_loss:',few_iou_loss.item(),'bg_loss:',bg_loss.item(),'total_loss:',total_loss.item())
        ...
        if not args.novalid:
            ...
            with torch.no_grad():
                for step, data in enumerate(val_loader):
                    ...
                    query_img, query_mask, support_img, support_mask, idx \
                        = query_img.cuda(), query_mask.cuda(), support_img.cuda(), support_mask.cuda(), idx.cuda()
                    pred_map = net(query_img,query_mask, support_img, support_mask)
                    pred_map = pred_map.squeeze(2)
                    query_mask = query_mask.squeeze(2)
