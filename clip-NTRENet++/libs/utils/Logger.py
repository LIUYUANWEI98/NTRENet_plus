...
class TreeEvaluation():
    ...
    def setup(self):
        ...
        self.vid_consistency_list = [0] * self.num_classes
        self.vc7_n_list = [0] * self.num_classes

        self.vid_consistency_new_list = [0] * self.num_classes
        self.vc7_new_n_list = [0] * self.num_classes
    ...
    def update_vc7(self, idx, query_mask, pred):
        # B N H W
        batch = len(idx)
        for i in range(batch):
            if not isinstance(idx[i], int):
                ...
                id = idx[i]
            id = self.class_indexes.index(id)
            consistency = video_consistency(query_mask[i], pred[i].cpu())
            if consistency is not None:
                self.vid_consistency_list[id] += consistency
                self.vc7_n_list[id] += 1
        self.vc7 = [self.vid_consistency_list[ic] / float(max(self.vc7_n_list[ic], 1)) for ic in range(self.num_classes)]

    def update_vc7_new(self, idx, query_mask, pred):
        # B N H W
        batch = len(idx)
        for i in range(batch):
            if not isinstance(idx[i], int):
                ...
                id = idx[i]
            id = self.class_indexes.index(id)
            consistency = video_consistency_new(query_mask[i], pred[i].cpu())
            if consistency is not None:
                self.vid_consistency_new_list[id] += consistency
                self.vc7_new_n_list[id] += 1
        self.vc7_new = [self.vid_consistency_new_list[ic] / float(max(self.vc7_new_n_list[ic], 1)) for ic in range(self.num_classes)]
...
def video_consistency(targets, preds, window=7):
    vcs = []
    if preds.shape[0] >= window:
        for i in range(preds.shape[0] - window + 1):
            common_gt_area = torch.ones(preds.shape[-2:])
            common_pred_area = torch.ones(preds.shape[-2:])
            previous_pred = None
            ...
            vcs.append((common_gt_area * common_pred_area).sum() / (common_gt_area.sum() + 1e-10))

    if len(vcs) != 0:
        vc = torch.stack(vcs).mean()
    else:
        vc = None
    return vc

def video_consistency_new(targets, preds, window=7):
    vcs = []
    if preds.shape[0] > window:
        for i in range(preds.shape[0] - window):
            common_gt_area = torch.ones(preds.shape[-2:])
            common_pred_area = torch.ones(preds.shape[-2:])
            for j in range(1, window):
                common = (targets[i] == targets[i + j]).float()
                common_gt_area = common * common_gt_area
                pred_common = (preds[i] == preds[i + j]).float()
                common_pred_area = pred_common * common_pred_area
            vcs.append((common_gt_area * common_pred_area).sum() / (common_gt_area.sum() + 1e-10))
